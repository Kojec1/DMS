...
class MHFacialLandmarkEstimator(nn.Module):
    def __init__(self, output_dims: dict = {'mpii': 6, 'wflw': 98, '300w': 68}, pretrained_backbone: bool = True, in_channels: int = 1, dropout_rate: float = 0.2) -> None:
        super(MHFacialLandmarkEstimator, self).__init__()
        self.backbone = convnext_tiny(pretrained=pretrained_backbone)
        self.in_channels = in_channels
        ...
        self.multi_head = nn.ModuleDict({
            'mpii': nn.Linear(self.backbone.out_features, self.output_dims['mpii'] * 2),
            'wflw': nn.Linear(self.backbone.out_features, self.output_dims['wflw'] * 2),
            '300w': nn.Linear(self.backbone.out_features, self.output_dims['300w'] * 2)
        })
    ...
    def forward(self, x: torch.Tensor, task: str) -> torch.Tensor:
        ...
        features_flattened = self.dropout(features_flattened)
        landmarks = self.multi_head[task](features_flattened)

        return landmarks
    

if __name__ == '__main__':
    model = MHFacialLandmarkEstimator(output_dims={'mpii': 6, 'wflw': 98, '300w': 68}, pretrained_backbone=False, in_channels=1, dropout_rate=0.2)
    model.eval()
    dummy_input = torch.randn(2, 1, 224, 224)
    landmarks = model(dummy_input, task='mpii')
    print(landmarks.shape)
    assert landmarks.shape == (2, 6 * 2)

    landmarks = model(dummy_input, task='wflw')
    print(landmarks.shape)
    assert landmarks.shape == (2, 98 * 2)

    landmarks = model(dummy_input, task='300w')
    print(landmarks.shape)
    assert landmarks.shape == (2, 68 * 2)